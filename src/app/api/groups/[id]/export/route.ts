  ...
  switch (fmt) {
    case 'pdf': {
      ...
      try {
        ...
        // Surface the underlying error instead of letting Next.js return
        // an opaque 500 HTML that the browser tries to save as ".pdf".
        // Body is JSON so the client receives a real error message.
        const message = e instanceof Error ? e.message : 'PDF_BUILD_FAILED';
        console.error('[export/pdf] build failed', {
          groupId,
          error: message,
          stack: e instanceof Error ? e.stack : undefined,
        });
        return NextResponse.json(
          { error: 'PDF_BUILD_FAILED', detail: message },
          { status: 500 },
        );
      }

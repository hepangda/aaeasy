...
const REGULAR_SUBPATH = path.join(
  '@expo-google-fonts',
  'noto-sans-sc',
  '400Regular',
  'NotoSansSC_400Regular.ttf',
);
const BOLD_SUBPATH = path.join(
  '@expo-google-fonts',
  'noto-sans-sc',
  '700Bold',
  'NotoSansSC_700Bold.ttf',
);

function resolvePackageFontPath(specifier: string): string | null {
  try {
...
/**
 * Walk `<root>/node_modules/.pnpm/@expo-google-fonts+noto-sans-sc@*` looking
 * for the regular/bold TTF inside. Needed because Vercel traces the *real*
 * font files (under `.pnpm/...`) into the function bundle but does not always
 * recreate the symlink at `node_modules/@expo-google-fonts/noto-sans-sc`, so
 * a plain `require.resolve('@expo-google-fonts/...')` fails at runtime.
 */
function findInPnpmStore(root: string, subpath: string): string | null {
  ...
  for (const entry of entries) {
    if (!entry.startsWith('@expo-google-fonts+noto-sans-sc@')) continue;
    const candidate = path.join(storeDir, entry, 'node_modules', subpath);
    if (existsSync(candidate)) return candidate;
  }
  return null;
}

function resolveBundledFont(subpath: string, packageSpecifier: string): string | null {
  const direct = resolvePackageFontPath(packageSpecifier);
  if (direct && existsSync(direct)) return direct;

  // Direct path via cwd — works when the symlink at `node_modules/@expo-
  // google-fonts/noto-sans-sc` is preserved in the deployed bundle.
  const cwdPath = path.join(process.cwd(), 'node_modules', subpath);
  if (existsSync(cwdPath)) return cwdPath;

  // Last resort: scan the pnpm content-addressable store under cwd.
  const pnpmPath = findInPnpmStore(process.cwd(), subpath);
...
function resolveCjkFontFiles(): PdfFontFiles | null {
  const bundledRegular = resolveBundledFont(
    REGULAR_SUBPATH,
    '@expo-google-fonts/noto-sans-sc/400Regular/NotoSansSC_400Regular.ttf',
  );
  const bundledBold = resolveBundledFont(
    BOLD_SUBPATH,
...
function ensurePdfFont(): string {
  ...
  if (!fontFiles) {
    throw new Error(
      'PDF_FONT_NOT_FOUND: @expo-google-fonts/noto-sans-sc TTFs were not located in the deployed bundle. ' +
        'Verify `outputFileTracingIncludes` traced the files and that the function has read access to them.',
    );
  }
